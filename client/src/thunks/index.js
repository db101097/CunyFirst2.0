...
export const addThunk = (classID, studentID) => dispatch => {
  return axios.post('http://localhost:8080/addClass/' + classID, {
    "studentId": studentID
  }).then(res => {
    console.log(res);
...
export const deleteThunk = (classID, studentID) => dispatch => {
  return axios.delete('http://localhost:8080/deleteClass/' + classID + '/' + studentID)
    .then(res => {
      console.log(res);
      window.alert('SUCCESS:\nClass Deleted')
      window.location.replace('/');
...
export const getScheduleThunk = studentID => dispatch => {
  return axios.get('http://localhost:8080/getSchedule/' + studentID)
    .then(res => {
